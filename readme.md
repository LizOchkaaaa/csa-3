...
| ++      | inc        |               Отсутствует               |       3       |
| --      | dec        |               Отсутствует               |       3       |
| [Слово] | call       |         Адрес начала процедуры          |       4       |
| null    | ret        |               Отсутствует               |       3       |

- Если за словом не следует !, @, !* или @*, то слово интерпретируется, как название процедуры.
- `ret` не сопоставляется какой-либо лексеме Forth. Данная инструкция подставляется в трансляторе в конце
процедуры.
...
Трансляция происходит в 2 цикла. В первом цикле транслируется код в процедурах, а во втором - основной код.
Если код содержит хотя бы одну процедуру, то первая инструкция (с индексом 0) будет представлять собой инструкцию
`jmp` на первую инструкцию основного кода.
...
7. В конце машинного кода вставляется инструкция `stop`.
...
- Вся память микроинструкций поделена на инструкции. Инструкции могут содержать разное количество микроинструкций.
    ...
    дующую микроинстр.
    - Практически в конце каждой инструкции (кроме stop, jmp, jif и call) находится микроинстр. перехода на следующую
...
- Ввод: `cat cat cat`
  ...
  - Вывод:
      ...
      DEBUG: (13: stop -> stop ) - TICK: 483 - TOS: 0 - data stack: [1, 0, 0, 0] - alu out: 0 - memory out: 0 - address reg: 0 - PC: 13 - mPC: 70
      cat cat cat 
      -------------------------------
...
| Суворова Е.М. | cat | 6 | 14 | 142 | 520 | (forth | stack | harv | mc | tick | struct | stream | mem | cstr | prob1)
| Суворова Е.М. | hello user name | 23 | 90 | 174 | 670 | (forth | stack | harv | mc | tick | struct | stream | mem | cstr | prob1)
```
